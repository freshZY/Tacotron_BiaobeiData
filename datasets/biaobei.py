...
def build_from_path(in_dir, out_dir, num_workers=1, tqdm=lambda x: x):
  ...
  futures = []
  index = 1
  with open(os.path.join(in_dir,'000001-010000.txt'),'rb') as f:
    data_list=f.readlines()
    pinyin_list=[]
    for i in range(1,len(data_list),2):
        pinyin_list.append(data_list[i])
    wav_list=[]
    for i in range(0,len(data_list),2):
        wav_list.append(os.path.join(in_dir,(str(data_list[i].split()[0],encoding='utf-8')+'.wav')))
